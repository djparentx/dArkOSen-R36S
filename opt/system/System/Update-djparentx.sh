...
LOG_FILE="/home/ark/djp-update.log"
...
wget -t 3 -T 60 --no-check-certificate "$LOCATION"/djparentxUpdate.sh -O /home/ark/djparentxUpdate.sh -a "$LOG_FILE" || sudo rm -f /home/ark/djparentxUpdate.sh
if [ $? -ne 0 ]; then
  sudo msgbox "Looks like OTA updating is currently down or your wifi or internet connection is not functioning correctly."
  printf "There was an error with attempting this update." | tee -a "$LOG_FILE"
  exit 1
...
sudo chmod -v 777 /home/ark/djparentxUpdate.sh | tee -a "$LOG_FILE"
/home/ark/djparentxUpdate.sh
...
if [ $? -ne 187 ]; then
  ...
  printf "There was an error with attempting this update." | tee -a "$LOG_FILE"
  if [ -f /home/ark/djparentxUpdate.sh ]; then
    rm /home/ark/djparentxUpdate.sh
  fi
fi
