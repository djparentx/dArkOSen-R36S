...
wget -t 3 -T 60 --no-check-certificate "$LOCATION"/dArkOSen-Update.sh -O /home/ark/dArkOSen-Update.sh -a "$LOG_FILE" || sudo rm -f /home/ark/dArkOSen-Update.sh
if [ $? -ne 0 ]; then
  sudo msgbox "Looks like OTA updating is currently down or your wifi or internet connection is not functioning correctly."
  printf "There was an error with attempting this update." | tee -a "$LOG_FILE"
  exit 1
...
sudo chmod -v 777 /home/ark/dArkOSen-Update.sh | tee -a "$LOG_FILE"
/home/ark/dArkOSen-Update.sh
...
if [ $? -ne 187 ]; then
  ...
  printf "There was an error with attempting this update." | tee -a "$LOG_FILE"
  if [ -f /home/ark/dArkOSen-Update.sh ]; then
    rm /home/ark/dArkOSen-Update.sh
  fi
fi
