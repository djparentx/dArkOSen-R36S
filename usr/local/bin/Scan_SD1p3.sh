...
export TERM=linux
CURR_TTY="/dev/tty1"
LOG_FILE="/boot/sd1p3_scan.log"
echo "Scan started at $(date '+%Y-%m-%d %H:%M:%S')" > "$LOG_FILE"

printf "===========================================\n"
printf "     SD1p3 Scan and Repair by djparent\n"
printf "===========================================\n"
exec > >(tee -a "$LOG_FILE") 2>&1

printf "\nSD1p3 scan started"
printf "\nUnmounting /dev/mmcblk0p3"

UNMOUNT_OK=1
for MP in $(mount | grep -E /dev/mmcblk0p3 | awk '{print $3}' | awk '{ print length, $0 }' | sort -rn | cut -d' ' -f2-); do
    printf "\nUnmounting %s (/dev/mmcblk0p3)\n" "$MP"
...
if [ "$UNMOUNT_OK" -eq 1 ]; then
    ...
    RESULT="failed"
    REPAIR_NOTES=("Could not unmount SD1p3")
fi

printf "Remounting via fstab"
mount -a 2>>"$LOG_FILE"
